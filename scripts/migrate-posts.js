/**
 * Migration script: populate Sanity with all 10 existing blog posts.
 * Run with:  node scripts/migrate-posts.js
 *
 * Requires NEXT_PUBLIC_SANITY_PROJECT_ID, NEXT_PUBLIC_SANITY_DATASET,
 * and SANITY_API_TOKEN (write-enabled) in the environment.
 * The token must have "Editor" rights on the dataset.
 */
...
if (!projectId) {
  console.error('❌  NEXT_PUBLIC_SANITY_PROJECT_ID not set. Add it to your .env.local and re-run.');
  process.exit(1);
}
if (!token) {
  console.error('❌  SANITY_API_TOKEN not set.');
  console.error('    Go to https://www.sanity.io/manage → Project → API → Tokens → Add API token (Editor)');
  console.error('    Then: $env:SANITY_API_TOKEN="your-token"; node scripts/migrate-posts.js');
  process.exit(1);
}

const client = createClient({ projectId, dataset, token, apiVersion: '2026-06-15', useCdn: false });

// ---------------------------------------------------------------------------
// Helper: convert our simple markdown-style text → Portable Text blocks
// ---------------------------------------------------------------------------
function textToPortableText(rawText) {
  ...
  for (const para of paragraphs) {
    ...
    if (/^\d+\.\s/.test(trimmed)) {
      ...
      for (const item of items) {
        blocks.push({
          ...
          level: 1,
          markDefs: [],
          children: [{ _type: 'span', _key: randomKey(), text: item.replace(/^\d+\.\s*/, ''), marks: [] }],
        });
      }
      continue;
    }

    // **Bold heading** — full paragraph is bold → treat as h2
    if (trimmed.startsWith('**') && trimmed.endsWith('**') && !trimmed.slice(2, -2).includes('**')) {
      blocks.push({
        ...
        style: 'h2',
        markDefs: [],
        children: [{ _type: 'span', _key: randomKey(), text: trimmed.slice(2, -2), marks: [] }],
      });
      continue;
    }
...
function parseInline(text) {
  ...
  return parts.map(part => {
    if (part.startsWith('**') && part.endsWith('**')) {
      return { _type: 'span', _key: randomKey(), text: part.slice(2, -2), marks: ['strong'] };
    }
    return { _type: 'span', _key: randomKey(), text: part, marks: [] };
  }).filter(s => s.text);
}
...
The fastest-growing eCommerce brands we work with are integrating micro-influencer partnerships into their content system — not as a separate strategy but as an extension of their owned content.`,
  },
  {
    ...
    tag: 'Paid Media',
    title: 'How to Fix Underperforming Google Ads Campaigns in 2025',
...
If your Google Ads are delivering under 5x ROAS, your account has structural issues. Book a free audit — we'll identify exactly what's costing you and show you the roadmap to 10x.`,
  },
  {
    ...
    publishedAt: '2025-05-15T01:00:00.000Z',
    readTime: '8 min read',
    color: 'var(--accent-peach)',
...
Target under 2 seconds on mobile. Use Next.js image optimization, lazy loading for below-fold content, and async font loading.

**Element 6: A Risk Reversal**

The biggest barrier to conversion is perceived risk. Risk reversal removes that barrier entirely. Risk reversals that work: money-back guarantees, free trials, free first session, "cancel anytime," performance-based pricing.

**Element 7: Urgency and Scarcity**

Urgency creates action. Without a reason to act now, most visitors will intend to come back — and never do. Ethical urgency includes: limited spots available this month, price increasing after a date, bonus available for a limited time.

**Element 8: A Visual Hierarchy That Guides the Eye**

Your page layout must guide visitors through a specific journey: headline → subheadline → social proof → benefits → CTA. Use size, color, and whitespace to create clear visual hierarchy.

**Element 9: Mobile-First Design**

Over 70% of ad traffic lands on mobile. A landing page designed for desktop and adapted for mobile will always underperform one designed mobile-first.

**The Results**

For Aurelia Jewellery, implementing these 9 elements on their campaign landing pages increased conversion rate from 1.4% to 6.2% — a 4.4x improvement with zero increase in ad spend.
...
3. Write 8-10 "Spoke" articles targeting highly specific long-tail keywords.
...
If you're spending on ads without a structured SEO strategy, you're leaving a high-yield asset unbuilt. Book a free strategy call — we'll analyze your search opportunity and map out your authority blueprint.`,
  ...
  {
    slug: 'scale-d2c-brand-meta-ads',
    tag: 'Paid Media',
    title: 'How to Scale a D2C Brand with Meta Ads in 2025',
    publishedAt: '2025-05-15T03:00:00.000Z',
    readTime: '10 min read',
    color: 'var(--accent-primary)',
    excerpt: "Scaling a D2C brand from Rs.5L to Rs.50L/month in Meta ad spend requires a specific framework. Here's the exact playbook we use — including the creative formula that consistently hits 17x ROAS.",
...
async function migrate() {
  console.log(`🚀 Starting migration to Sanity project ${projectId} / dataset ${dataset}`);
  ...
  for (const post of posts) {
    ...
    const body = textToPortableText(post.content);
